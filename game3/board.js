...
function closeInputDiv(){
		$("#inputBlack").fadeOut();
		$("#"+activeInputDiv).fadeOut();
}

function addPlayerMode(a,b){
	$("#player"+a+"ai").val(b);
	if (b==1) {
		$("#playerModeLabel"+a).html('Computer');
	}else{
		$("#playerModeLabel"+a).html('Human');
	}
	closeInputDiv();
}

function addPlayerToken(a,b){
	$("#player"+a+"icon").val(b);
	$("#playerTokenLabel"+a).html('<img src="assets/token'+b+'.png">');
