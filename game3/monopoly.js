...
function Player(name, color, icon) {
	this.name = name;
	this.color = color;
	this.icon = icon;
	this.oldposition = 0;
...
function popup(HTML, action, option) {
	...
	// Ok
		...
		$("#popuptext").append("<div><input class='setup-btn' type='button' value='OK' id='popupclose' /></div>");
		$("#popupclose").focus();
...
function updatePosition() {
	...
	moveImageWithAnimation(player[turn].icon, player[turn].oldposition, player[turn].position);
...
function updateMoney() {
	...
	document.getElementById("pmoney").innerHTML = "D" + p.money;
	// $(".money-bar-row").hide();
	$(".player-stat").hide();
	...
	for (var i = 1; i <= pcount; i++) {
		...
		// $("#moneybarrow" + i).show();
		$("#p" + i + "stat").show();
		document.getElementById("p" + i + "moneyname1").style.backgroundColor = p_i.color;
		document.getElementById("p" + i + "moneyname1").innerHTML = p_i.name;
		document.getElementById("p" + i + "money1").innerHTML = "D"+p_i.money;
		document.getElementById("p" + i + "icon1").src = "assets/token"+p_i.icon+".png";
...
// switch auction back on
	...
	$("#table, #control, #playerStats, #viewstats, #buy").show();
...
function setup() {
	...
	$("#table, #playerStats, .top-btns").show();
	$("#setup").hide();
...
function moveImageWithAnimation(icon, oldposition, position) {
    var image = document.getElementById('icon'+icon);
    var targetDiv = document.getElementById('cell'+position);
