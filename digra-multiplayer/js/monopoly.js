...
if (urlParams.has('player_turn')) {
	player_turn = parseInt(urlParams.get('player_turn'));
...
function Game() {
	...
	this.acceptTrade = function(tradeObj) {
		...
		closeCpanel();
		aa("https://cdn.lordicon.com/dvjujnbv.json", initiator.name + " and " + recipient.name + " have made a trade.", 100, 100, 10000);
	...
	this.bankruptcy = function() {
		...
		if (pcount === 2 || bankruptcyUnmortgageFee === 0 || p.creditor === 0) {
			...
			popup("<p>" + pcredit.name + ", you must pay D" + bankruptcyUnmortgageFee + " interest on the mortgaged properties you received from " + p.name + ".</p>", function() {player[pcredit.index].pay(bankruptcyUnmortgageFee, 0); game.bankruptcyUnmortgage();});
		}
	};
...
function sortBoard() {
	...
	for (var i = 0; i < lbplayers.length; i++) {
		document.getElementById("sb" + lbplayers[i].avatar).style.bottom = bottomdistance + "px";
		bottomdistance += 60;
	}
}
...
function updateMoney() {
	...
	for (var i = 1; i <= pcount; i++) {
		p_i = player[i];
		document.getElementById("avatar" + p_i.avatar + "money").innerHTML = p_i.name +" <br>D"+p_i.money;
		document.getElementById("avatar" + p_i.avatar + "money").dataset.score = p_i.money;
	}
	...
	for (var i = 0; i < 40; i++) {
		...
		if (sq.owner == turn) {
			...
			if (sq.mortgage) {
				...
				HTML += "' for='propertycheckbox"+i+"'><div class='playerstats-tile property-cell-row'><input type='checkbox' id='propertycheckbox" + i + "'></td><div class='playerstats-tile-color' style='background: " + sq.color + ";'></div><div class='playerstats-tile-name'>"+sq.name+"</div>";
				
				if (sq.hotel) {
					HTML +="<img src='images/law.png'><div class='pst-name'>"+sq.hotel+"</div></div></label><br>";
				}else{
					HTML +="<img src='images/policy.png'><div class='pst-name'>"+sq.house+"</div></div></label><br>";
				}
		...
		for (var i = 0; i < 40; i++) {
			...
			if (sq.owner == x) {
				...
				HTML += "<div class='playerstats-tile-name'>"+sq.name+"</div>";
					...
					HTML += "<img src='images/policy.png' style='height:15px'><div class='pst-name'>" + sq.house + "</div>";
				}
...
function updateGame(){
	...
	pcount = snapshot.val().playerCount;
	turn = snapshot.val().turn;
	doublecount = snapshot.val().doublecount;
	lastShownCity = snapshot.val().lastShownCity;

	player_new = snapshot.val().player;
	square = snapshot.val().square;

	chanceCards.index = snapshot.val().chanceCardsIndex;
	communityChestCards.index = snapshot.val().communityChestCardsIndex;

	chanceCards.deck = snapshot.val().chanceCardsDeck;
	communityChestCards.deck = snapshot.val().communityChestCardsDeck;

	pensionerFirstRent = snapshot.val().pensionerFirstRent;
	caregiverFirstRow = snapshot.val().caregiverFirstRow;
	studentFirstRow = snapshot.val().studentFirstRow;

	first_load = snapshot.val().first_load;
	last_action = snapshot.val().last_action;
	areDiceRolled = snapshot.val().areDiceRolled;
	...
	for (var i = 1; i <= pcount; i++) {
		...
		player[i].bidding = player_new[i].bidding;
		player[i].chanceJailCard = player_new[i].chanceJailCard;
		player[i].communityChestJailCard = player_new[i].communityChestJailCard;
		player[i].creditor = player_new[i].creditor;
		player[i].index = player_new[i].index;
		player[i].jail = player_new[i].jail;
		player[i].jailroll = player_new[i].jailroll;
		player[i].money = player_new[i].money;
		player[i].oldposition = player_new[i].oldposition;
		player[i].position = player_new[i].position;

		
		$("#avatar"+player[i].avatar).css({"left": positions[player[i].position][0]+"px", "top": positions[player[i].position][1]+"px"});
		
		// check if another player is on the same position
		var new_top = positions[player[i].position][1];
		for (var j = 1; j <= pcount; j++) {
			if (j !== turn && player[j].position === player[turn].position) {
				new_top -= 20;
				$("#avatar"+player[j].avatar).css({"left": positions[player[j].position][0]+"px", "top": new_top+"px"});
			}else{
				new_top = positions[player[j].position][1];
			}
		}
		document.getElementById("avatar" + p.avatar + "money").innerHTML = player[i].name +" <br>D"+player[i].money;
		document.getElementById("avatar" + p.avatar + "money").dataset.score = player[i].money;
...
function uploadGameData(){
	if(1){
		...
		update(ref(db, "game/"+ gameID), data)
		.then(()=>{
...
window.onload = function() {
	...
	for (var i = 1; i <= pcount; i++) {
		...
		$("#sb"+playerArray[i].avatar).css("display", "flex");
